...
class CheckpointRunner(AllCheckpointing, GenericRunner):
    ...
    def append_surface_data(self, points, faces, uvs, surface_type, name_tags, ckpt_info, scalars=None, colors=None, constant=False):
        if (not constant) or (constant and ckpt_info.save_constant_data):
            ...
            self.types_list.append(surface_type)
    ...
    def end_checkpointing(self, model, name, ckpt_info):
        self.save_surfaces_from_list(self.points3D_list, self.faces_list, self.uvs_list, self.types_list, self.scalars_list, self.colors_list, self.names, ckpt_info.checkpoint_dir)

        self.save_model(ckpt_info.checkpoint_dir, model)
