...
class ConfigPathGenerator():
    ...
    def compose_config_path(self):
        ...
        self.config['logging']['folder'] = log_folder

        self.config['checkpointing']['folder'] = self.config['checkpointing']['base_path'] + namespace
    ...
    def stringify_experiment(self):
        model_str   = self.stringify_model(self.config['model'])
        dataset_str = self.stringify_dataset(self.config['dataset'])
        ...
        if ('load_checkpoint', True) in self.config['model'].items():
            out_str += '_restart'
    ...
    def stringify_dataset(self, config):
        out_str = ''
        if 'batch_size' in config and ('batch_size', None) not in config.items():
            if config['batch_size'] > 1:
                out_str += 'BS-' + str(config['batch_size'])
        if 'num_points' in config:
            if len(out_str) > 1:
                out_str += '_'
            out_str += 'NP-' + str(config['num_points'])
