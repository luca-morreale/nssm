...
#vit features extractor that runs only up to the desired layer.
class ViTExtractor_Fast:
    ...
    def __init__(self, model_type: str = 'dino_vits8', stride: int = 16, model: nn.Module = None, device: str = 'cuda'):
        ...
        if model is not None:
            ...
            self.model = ViTExtractor_Fast.create_model(model_type)
        self.model = ViTExtractor_Fast.patch_vit_resolution(self.model, stride=stride)
        self.model.eval()
    ...
    def patch_vit_resolution(model: nn.Module, stride: int) -> nn.Module:
        ...
        model.interpolate_pos_encoding = types.MethodType(ViTExtractor_Fast._fix_pos_enc(patch_size, stride), model)
        return model

    def old_preprocess(self, image_path: Union[str, Path],
    ...
    def preprocess(self, image: Union[torch.Tensor, np.ndarray, str], *args) -> Tuple[torch.Tensor, Image.Image]:
        if type(image)==str:
            return self.old_preprocess(image, [224,244])
        prep_img = self.transform(image)
        return prep_img# , pil_image


    def forward(self, x, layers: List[int], facet: str):
        if len(layers) == 1:
            layer = layers[0]
        ...
        for i, blk in enumerate(self.model.blocks):
            ...
            if i in layers:
                ...
                x = blk.norm1(x)
                x = blk.attn.qkv(x)
                x = x.reshape(B, N, 3, blk.attn.num_heads, C // blk.attn.num_heads).permute(2, 0, 3, 1, 4)
                self._feats.append(x[facet_idx])
                q, k, v = x[0], x[1], x[2]

                attn = (q @ k.transpose(-2, -1)) * blk.attn.scale
                attn = attn.softmax(dim=-1)
                attn = blk.attn.attn_drop(attn)
                self._saliency.append(attn)
    ...
    def _extract_features(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key') -> List[torch.Tensor]:
        ...
        #     features_dict = self.model.forward_features(batch)
        #     self._feats = [features_dict['x_norm_patchtokens'].unsqueeze(0)]
        # else:
        #     self._feats = self.forward(batch, layers, facet)
        self._feats = self.forward(batch, layers, facet)
        ...
        if type(p) == tuple:
            # print_info(f'p {p}')
            p = p[0]
        # print_info(f'H {H} W {W} stride {self.stride} p {p}')
        ...
        # print_info(f'feats {self._feats[0].shape}')
        # print_info(f'num_patches {self.num_patches}')
        return self._feats
    ...
    def extract_descriptors(self, batch: torch.Tensor, layer: int = 11, facet: str = 'key',
        ...
        # print_info(f'x {x.shape}')
        if not bin:
            desc = x.permute(0, 2, 3, 1).flatten(start_dim=-2, end_dim=-1).unsqueeze(dim=1)  # Bx1xtx(dxh)
        else:
            desc = self._log_bin(x)

        # print_info(f'desc {desc.shape}')
        return desc
    ...
    def extract_saliency_maps(self, batch: torch.Tensor) -> torch.Tensor:
        ...
        self._extract_features(batch, [11], 'key') # here the facet does not matter, we use only the attention layer
        head_idxs = [0, 2, 4, 5]
        # curr_feats = self._feats[0] #Bxhxtxt
        curr_feats = self._saliency[0] #Bxhxtxt
        cls_attn_map = curr_feats[:, head_idxs, 0, 1:].mean(dim=1) #Bx(t-1)
        temp_mins, temp_maxs = cls_attn_map.min(dim=1)[0].unsqueeze(-1), cls_attn_map.max(dim=1)[0].unsqueeze(-1)
